...
// CODE HERE
const restyleHelloBtn = () => {
    sayHelloButton.classList.add('reverse-btn')
}

sayHelloButton.addEventListener('mouseover', restyleHelloBtn)
...
// CODE HERE
const resetHelloBtn = () => {
    sayHelloButton.classList.remove('reverse-btn')
}

sayHelloButton.addEventListener('mouseout', resetHelloBtn)
...
const makeRequest = () => {
    ...
    axios
    .get(`${baseURL}/query-test?name=kellon`)
    .then((res) => {
        console.log(res.data)
    })
    .catch((err) => {
...
document.querySelector('#query-button').addEventListener('click', makeRequest)
...
/*
    ...
    2: Send more than 1 query on the URL -- what happened? 
        // You sent query: name with value: kellon?food=icecream!
*/
